    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/iamajaygaur/BankingApp.git'
            }
		}
        stage('Maven Build') {
            steps {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t iamajaygaur/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag iamajaygaur/bankapp-eta-app:${BUILD_NUMBER} iamajaygaur/bankapp-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push iamajaygaur/bankapp-eta-app:latest"
			}
		}
